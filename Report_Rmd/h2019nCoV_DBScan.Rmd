...
# 1.样本meta信息
```{r,include=FALSE}
datastat <- read.table("/output/metadata.csv")
```
...
# 2. 一致性序列QC信息
```{r,include=FALSE}
datastat <- read.table("/output/fasta.qc.xls")
```
...
# 3. 一致性序列变异信息
```{r,include=FALSE}
datastat <- read.table("/output/variants/vcMerged.xls")
```
...
# 4. 一致性序列变异信息总结
```{r,include=FALSE}
datastat <- read.table("/output/variants.summary.xls")
```
...
# 5. Top10最相似序列
```{r,include=FALSE}
datastat <- read.table("/output/similarity.based_on_variant_calling.top10.xls")
```
...
```{r,include=FALSE}
datastat <- read.table("/output/vcMerged.xls")
```
